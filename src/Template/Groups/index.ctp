...
<section class="content-header">
    <div class="row">
        <div class="col-xs-12 col-md-6">
            <h4><?= __d('Qobo/Groups', 'Groups'); ?></h4>
        </div>
        <div class="col-xs-12 col-md-6">
            <div class="pull-right">
                <div class="btn-group btn-group-sm" role="group">
                <?= $this->Html->link(
                    '<i class="fa fa-plus"></i> ' . __d('Qobo/Groups', 'Add'),
                    ['plugin' => 'Groups', 'controller' => 'Groups', 'action' => 'add'],
                    ['escape' => false, 'title' => __d('Qobo/Groups', 'Add'), 'class' => 'btn btn-default']
                ); ?>
                </div>
            </div>
        </div>
    ...
    <div class="box box-primary">
        <div class="box-body">
            <table class="table table-hover table-condensed table-vertical-align table-datatable">
                <thead>
                    <tr>
                        <th><?= __d('Qobo/Groups', 'Name') ?></th>
                        <th><?= __d('Qobo/Groups', 'Users') ?></th>
                        <th class="actions"><?= __d('Qobo/Groups', 'Actions') ?></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            ...
                            <?= $this->Html->link(
                                ...
                                ['title' => __d('Qobo/Groups', 'View'), 'class' => 'btn btn-default btn-sm', 'escape' => false]
                            ); ?>
                            <?php if (!$group->deny_edit) : ?>
                                <?= $this->Html->link(
                                    '<i class="fa fa-pencil"></i>',
                                    ['plugin' => 'Groups', 'controller' => 'Groups', 'action' => 'edit', $group->id],
                                    ['title' => __d('Qobo/Groups', 'Edit'), 'class' => 'btn btn-default btn-sm', 'escape' => false]
                                ); ?>
                            <?php endif; ?>
                            <?php if (!$group->deny_delete) : ?>
                                <?= $this->Form->postLink(
                                    ...
                                    [
                                        'confirm' => __d('Qobo/Groups', 'Are you sure you want to delete {0}?', $group->name),
                                        'title' => __d('Qobo/Groups', 'Delete'),
